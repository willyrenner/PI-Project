...
$agendList = $model->listAgends($_SESSION['user']);
?>
    ...
    <section id="hero" class="position-relative overflow-hidden py-4" style="background: linear-gradient(rgba(0,0,0,0.8), rgba(0,0,0,0.8)),url(/images/banner-img.jpg)">
        <div class="container py-5">
            <div class="row justify-content-center align-items-center py-5">
                <div class=" col-md-5">
                    <h1 class="text-white">Seus Horários Agendados</h1>
                    <?php 
                        if (!empty($agendList)) {
                            ...
                            foreach ($agendList as $agend) {
                                echo "<li style='margin-bottom: 10px; color:white;'>Nome: {$agend['nome_cliente']} <br>Data: {$agend['data_consulta']} <br>Horário: {$agend['hora_consulta']}</i></li>";
                            }
                            
                            echo '<a class="btn" href="/remove">Remover Agendamento</a>';
                            echo "</ul>";
                        } else {
                    ...
                    ?>
                    <li class="nav-item">
                        <a href="/dashboard"><button type="button" class="btn btn-danger">Voltar</button></a>
                    </li>
                </div>
            </div>
        </div>
