        ...
        <div class="container py-5">
            <div class="row justify-content-center align-items-center py-5">
                <div class="col-md-4">
                    ...
                    <?php 
                        if (!empty($userList)) {
                            echo "<ul style='list-style-type: none; padding: 20px; border: px solid white; background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5))';>";
                            foreach ($userList as $user) {
                                echo "<li style='margin-bottom: 10px; color:white;'>ID: {$user['id']} <br>Usuário: {$user['user']} <br>Email: {$user['email']} <i>{$user['typeUser']}</i></li>";
                            }
                            echo "</ul>";
                        } else {
